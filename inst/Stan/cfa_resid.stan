...
parameters {
  ...
  vector[N_complex] loadings_cmplx;
  vector<lower = 0>[complex_struc] sigma_loadings_cmplx;
  vector<lower = 2.0>[complex_struc] gdp_loadings_cmplx;
}
model {
  ...
  if (complex_struc == 1) {
    sigma_loadings_cmplx ~ std_normal();
    gdp_loadings_cmplx ~ lognormal(1, 1);
    target += generalized_double_pareto_lpdf(
      loadings_cmplx | gdp_loadings_cmplx[1]);
  }
      ...
      for (i in 1:Ni) {
        for (j in 1:Nf) {
          if (loading_pattern[i, j] != 0) {
            ...
            pos_cmplx += 1;
            Load_mat[i, j] = sigma_loadings_cmplx[1] * loadings_cmplx[pos_cmplx];
          }
        }
      }
    }
