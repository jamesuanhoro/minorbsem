...
- provides a test of absolute misfit and estimates an effect size for the degree of model misfit.
...
The influence of minor factors is captured by estimating the residual covariances between all items, or relaxing the local independence assumption in SEMs. Unlike @Muthen2012, minorbsem is able to estimate both pre-specified residual covariances and the full residual covariance matrix without problems. Moreover, the influence of minor factors can be estimated under two assumptions:
...
minorbsem returns posterior predictive p-values [PPP,@Meng1994;@Muthen2012] which replace $\chi^2$-tests of absolute fit in the Bayesian context. The ideal PPP is .5, with values closer to 0 suggesting significant misfit -- @Muthen2012 use a .05 as a threshold for inadequate fit. Additionally, each estimation method in minorbsem returns an effect size fit index that is estimated as part of the model. For most approaches implemented in minorbsem, this index is analogous to the correlation root mean square residual [CRMR,@ogasawara_standard_2001].
...
In the next section, we demonstrate the main features and functions of minorbsem using an applied example. We then discuss how minorbsem compares to existing SEM packages that account for minor factors. Then we conclude with some planned updates for the package.
...
We will need the `devtools` package to install R packages that are hosted on GitHub instead of Cran (the default repository for R packages):
...
Afterwards, the user needs to install Stan itself. First, we check that the computer is correctly set up to install Stan:

```{r eval=FALSE}
cmdstanr::check_cmdstan_toolchain()
```

Afterwards, the user can then install Stan:

```{r eval=FALSE}
cmdstanr::install_cmdstan()
```

Once Stan is installed, then the user can install minorbsem:
...
We analyze the popular Holzinger-Swineford dataset using confirmatory factor analysis (CFA) to demonstrate the features of minorbsem. First we load the package:
...
The basic model syntax for the data is:
...
```{r echo = FALSE, message=1:5, cache=TRUE}
hs_output <- capture.output(
  hs_fit_1 <- minorbsem(model = hs_syntax_1, data = HS, show = FALSE)
)
```

```{r echo = FALSE}
writeLines(hs_output[c(1:5, 42:46)])
```
...
kableExtra::save_kable(
  pretty_print_summary(hs_fit_1),
  file = "hs_1.png", zoom = 1.5
)
```

```{r echo = FALSE}
# knitr::include_graphics("hs_1.png")
```
