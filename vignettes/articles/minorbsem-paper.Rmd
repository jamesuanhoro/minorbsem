...
Model results are saved in the object, `hs_fit_1`, and automatically displayed in a window of the user's default web browser. If the user is working within RStudio, the results are displayed in the Viewer pane of RStudio. To disable this automatic viewing of results, set `show = FALSE` when calling `minorbsem()`. Another way to view parameter estimates for major parameters is: `hs_fit_1@major_parameters` which prints the model results in the console.
...
Model results are displayed exactly as shown in Table \@ref(tab:tab-1). The parameter are divided into sections: 
