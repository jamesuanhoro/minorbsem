...
For this example, we assume the following DAG:
...
First, we need to create dummy variables for the `grade`. All eighth graders are coded 1 and seventh graders are all 0.

```{r}
HS$grade_8 <- ifelse(HS$grade == "8", 1, 0)
```

In the first syntax, there is only one predictor `age`.
